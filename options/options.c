...
// ------------------------- demuxer options --------------------
    ...
    {"prefetch-playlist", OPT_BOOL(prefetch_open), .flags = UPDATE_DEMUXER},
    {"prefetch-playlist-max", OPT_INT(prefetch_open_max),
        M_RANGE(1, INT_MAX), .flags = UPDATE_DEMUXER},
    {"prefetch-playlist-on-cache", OPT_BOOL(prefetch_open_on_cache)},
    {"prefetch-playlist-realtime", OPT_BOOL(prefetch_open_realtime)},
    {"prefetch-playlist-cache-secs", OPT_DOUBLE(prefetch_open_secs),
        M_RANGE(0, DBL_MAX), .flags = UPDATE_DEMUXER},
    {"prefetch-playlist-cache-bytes", OPT_BYTE_SIZE(prefetch_open_bytes),
        M_RANGE(0, M_MAX_MEM_BYTES), .flags = UPDATE_DEMUXER},
    {"prefetch-playlist-start-secs", OPT_DOUBLE(prefetch_open_start_secs),
        M_RANGE(0, DBL_MAX), .flags = UPDATE_DEMUXER},
    {"prefetch-playlist-start-bytes", OPT_BYTE_SIZE(prefetch_open_start_bytes),
        M_RANGE(0, M_MAX_MEM_BYTES), .flags = UPDATE_DEMUXER},
...
static const struct MPOpts mp_default_opts = {
    ...
    .prefetch_open_max = 1,
    .prefetch_open_start_secs = 10.0,
    .prefetch_open_start_bytes = 32 * 1024 * 1024,
    .hls_bitrate = INT_MAX,
    .cache_pause = true,
