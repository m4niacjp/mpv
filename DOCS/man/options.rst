...
``--prefetch-playlist-max=<N>``
    ...
    so that playlist-next can begin without waiting for the full cache cap.
...
``--prefetch-playlist-start-secs=<seconds>``
    ...
    expanding toward ``--prefetch-playlist-cache-secs`` (default: 10).
    Further ``--prefetch-playlist-max`` entries wait until this start window
    is full. 0 together with ``--prefetch-playlist-start-bytes=0`` restores
    filling the full prefetch cache caps immediately.

``--prefetch-playlist-start-bytes=<bytesize>``
    Byte cap for the immediate next entry's start window before expanding
    toward ``--prefetch-playlist-cache-bytes`` (default: 32MiB). 0 together
    with ``--prefetch-playlist-start-secs=0`` disables the start window.
...
``--autocreate-playlist=<no|filter|same>``
    ...
    The ``playlist`` property may still have a single entry at ``file-loaded``
    and then grow as siblings are inserted. ``--prefetch-playlist`` can start
    as soon as the next sibling exists.
